...
namespace qmapcontrol
{
    ...
    void MapControl::setSelectRect(int x, int y, int w, int h)
    {
        selectRect.setRect(x,y,w,h);
    }

    QRectF MapControl::getSelectRect()
    {
        return selectRect;
    }

    void MapControl::setSelectOrigin(int x, int y)
    {
        selectRect.setX(x);
        selectRect.setY(y);
    }

    void MapControl::setSelectSize(int w, int h)
    {
        selectRect.setWidth(w);
        selectRect.setHeight(h);
    }
    ...
    void MapControl::positionChanged(Geometry* geom)
    {
        ...
        QPoint dest = layermanager->layer()->mapadapter()->coordinateToDisplay(((QmapPoint*)geom)->coordinate());
    ...
    void MapControl::paintEvent(int mode)
    {
    ...
    void MapControl::paintEvent(QPaintEvent* evnt)
    {
       ...
       //qDebug("paointEvent");
        ...
        if (scaleVisible)
        {
            ...
            distanceList<<5000000<<2000000<<1000000<<1000000<<1000000<<100000<<100000<<50000<<50000<<10000<<10000<<10000<<1000<<1000<<500<<200<<100<<50<<30<<10<<5;
            if (currentZoom() >= 0 && distanceList.size() > currentZoom())
            {
                double line;
                line = distanceList.at( currentZoom() ) / pow(2, 17-currentZoom() ) / 0.597164;

                // draw the scale
                painter.setPen(Qt::black);
                QPoint p1(10,size.height()-20);
                QPoint p2((int)line-10,size.height()-20);

                painter.drawLine(p1,p2);

                painter.drawLine(10,size.height()-15, 10,size.height()-25);
                painter.drawLine((int)line-10,size.height()-15, (int)line-10,size.height()-25);
    ...
    void MapControl::mouseMoveEvent(QMouseEvent* evnt)
    {
        ...
        else if (mousepressed && mymousemode == Dragging)
        {
            ...
            QRectF rect = QRectF(clickToWorldCoordinate( pre_click_px), clickToWorldCoordinate( current_mouse_pos));
            selectRect = rect;
            emit sendSelectRect(rect);
            update();
        }
        // emit(mouseEventCoordinate(evnt, clickToWorldCoordinate(evnt->pos())));
    ...
    QPoint MapControl::worldToScreen(QPointF world)
    {

        // image coordinate to world coordinate
        return layermanager->layer()->mapadapter()->coordinateToDisplay(world);
    }
    ...
    void MapControl::setView(const QmapPoint* point) const
    {
    ...
    void MapControl::removeLayer(qint32 layerNumber)
    {
        layermanager->removeLayer(layerNumber);
    }
